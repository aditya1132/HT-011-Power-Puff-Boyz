...
class EmotionKeywords:
    ...
    EMOTION_PATTERNS = {
        "stressed": {
            "keywords": [
                "stressed", "pressure", "overwhelmed", "burden", "deadline",
                "anxiety", "panic", "worried", "tense", "exhausted",
                "can't handle", "too much", "breaking point", "falling behind"
            ],
            "phrases": [
                r"feel\s+stressed", r"under\s+pressure", r"so\s+much\s+work",
                r"can't\s+cope", r"burning\s+out", r"at\s+my\s+limit"
            ],
        ...
        "anxious": {
            "keywords": [
                "anxious", "nervous", "worry", "fear", "scared", "afraid",
                "panic", "restless", "uneasy", "apprehensive", "jittery",
                "what if", "catastrophic", "worst case", "can't stop thinking"
            ],
            "phrases": [
                r"feel\s+anxious", r"can't\s+stop\s+worrying", r"panic\s+attack",
                r"racing\s+thoughts", r"heart\s+racing", r"sweaty\s+palms"
            ],
            "intensifiers": ["extremely", "really", "so", "very", "incredibly"],
            "weight": 1.0
        },
        ...
        "sad": {
            "keywords": [
                ...
                "unhappy", "miserable", "heartbroken", "disappointed",
                "crying", "tears", "empty", "lonely", "hopeless"
            ],
            "phrases": [
                r"feel\s+sad", r"feeling\s+down", r"can't\s+stop\s+crying",
                r"feel\s+empty", r"so\s+alone", r"lost\s+hope"
            ],
        ...
        "overwhelmed": {
            "keywords": [
                ...
                "swamped", "buried", "crushed", "suffocated", "flooded",
                "overloaded", "breaking point", "at capacity"
            ],
            "phrases": [
                r"feel\s+overwhelmed", r"too\s+much\s+to\s+handle",
                r"drowning\s+in", r"can't\s+keep\s+up", r"falling\s+behind"
            ],
        ...
        "angry": {
            "keywords": [
                ...
                "frustrated", "pissed", "livid", "outraged", "fed up",
                "hate", "disgusted", "resentful"
            ],
            "phrases": [
                r"so\s+angry", r"fed\s+up", r"can't\s+stand", r"makes\s+me\s+mad",
                r"losing\s+my\s+temper", r"want\s+to\s+scream"
            ],
            "intensifiers": ["extremely", "really", "so", "very", "incredibly"],
            "weight": 0.9
        },
        ...
        "excited": {
            "keywords": [
                ...
                "amazing", "awesome", "fantastic", "wonderful", "great",
                "love", "happy", "joy", "delighted", "pumped"
            ],
            "phrases": [
                r"so\s+excited", r"can't\s+wait", r"over\s+the\s+moon",
                r"feel\s+amazing", r"best\s+day\s+ever"
            ],
            "intensifiers": ["extremely", "really", "so", "very", "incredibly"],
            "weight": 0.8
        },
        ...
        "positive": {
            "keywords": [
                "good", "fine", "okay", "alright", "decent", "content",
                "satisfied", "peaceful", "calm", "grateful", "thankful",
                "blessed", "optimistic", "hopeful"
            ],
            "phrases": [
                r"feel\s+good", r"doing\s+well", r"things\s+are\s+okay",
                r"feeling\s+better", r"grateful\s+for"
            ],
        ...
        "neutral": {
            "keywords": [
                "neutral", "normal", "average", "routine", "typical",
                "nothing special", "same as usual", "meh", "whatever"
            ],
            "phrases": [
                r"nothing\s+special", r"same\s+as\s+usual", r"pretty\s+normal",
                r"just\s+okay", r"not\s+much\s+happening"
            ],
            "intensifiers": [],
            "weight": 0.5
        },
        ...
        "confused": {
            "keywords": [
                ...
                "bewildered", "mixed up", "don't understand", "not sure",
                "complicated", "conflicted"
            ],
            "phrases": [
                r"don't\s+understand", r"not\s+sure\s+what", r"feel\s+lost",
                r"mixed\s+feelings", r"don't\s+know\s+what"
            ],
        ...
        "grateful": {
            "keywords": [
                "grateful", "thankful", "blessed", "appreciate", "lucky",
                "fortunate", "thank you", "thanks", "bless", "appreciate"
            ],
            "phrases": [
                r"feel\s+grateful", r"so\s+thankful", r"feel\s+blessed",
                r"appreciate\s+that", r"lucky\s+to\s+have"
            ],
...
class RuleBasedEmotionDetector:
    ...
    def detect_emotion(self, text: str) -> EmotionResult:
        ...
        try:
            ...
            emotion_scores = self._apply_sentiment_weighting(emotion_scores, sentiment_scores)
    ...
    def _analyze_sentiment(self, text: str) -> Dict[str, float]:
        ...
        try:
            ...
            return {'compound': 0.0, 'positive': 0.0, 'negative': 0.0, 'neutral': 1.0, 'polarity': 0.0, 'subjectivity': 0.0}

    def _calculate_emotion_scores(self, text_clean: str, text_lower: str) -> Dict[str, float]:
    ...
    def _apply_sentiment_weighting(self, emotion_scores: Dict[str, float], sentiment: Dict[str, float]) -> Dict[str, float]:
    ...
    def _get_secondary_emotions(self, emotion_scores: Dict[str, float], primary_emotion: str) -> List[Tuple[str, float]]:
...
class MLEmotionDetector:
    ...
    def detect_emotion(self, text: str) -> EmotionResult:
        ...
        if not self.is_available:
            logger.warning("ML emotion detection not available, falling back to rule-based")
...
class EmotionDetectionService:
    ...
    def analyze_emotion(self, text: str, method: str = None) -> EmotionResult:
        ...
        Args:
            text: Input text to analyze
            method: Detection method ('rule_based', 'ml', 'auto')
        ...
        try:
            if method == "ml" and self.ml_detector.is_available:
    ...
    def get_emotion_insights(self, emotion_result: EmotionResult) -> Dict[str, Any]:
        ...
        insights = {
            ...
            "needs_intervention": category == "negative" and intensity in ["high", "extreme"],
    ...
    def _get_suggested_tools(self, emotion: str) -> List[str]:
        ...
        tool_mapping = {
            "stressed": ["breathing_exercise", "grounding_technique", "progressive_relaxation"],
            "anxious": ["breathing_exercise", "mindfulness", "grounding_technique"],
            "sad": ["journaling", "gratitude_practice", "gentle_movement"],
            "overwhelmed": ["priority_setting", "breathing_exercise", "break_down_tasks"],
